...
p. A couple weeks ago we launched blog.spokt.com (wordpress) and we wanted to have links to recent blog posts in the footer at spokt.com (ruby on rails):

!/images/posts-in-footer.png(recent blog posts listed in spokt.com footer)!

p. To accomplish this I created a url at spokt which when accessed would download the blog's rss feed, parse it, and store it for later listing.  I then used my neearly-zero wordpress & php experience to create plugin that just curl'd the url.  Here's the plugin code:
