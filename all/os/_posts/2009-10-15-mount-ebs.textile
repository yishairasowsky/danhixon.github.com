...
p. In RightScale click "Clouds" then Navigate to AWS US -> EBS Volumes - New.  Fill out the form (I needed 10GB - it will also be easier to follow along here if you choose /dev/sdm).  Navigate to the volume you created and click the "Servers" tab.  Click the "Add server for boot attachment (At the bottom of the screen) then tell it which server to use.
